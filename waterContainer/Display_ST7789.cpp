...
void LCD_DrawStream(Stream *s, uint32_t len)
{             
  uint16_t Xstart = 0;
  uint16_t Ystart = 0;
  uint16_t Xend = LCD_WIDTH - 1; 
  uint16_t Yend = LCD_HEIGHT - 1;
  ...
  while(remaining > 0) {
      ...
      size_t readCount = s->readBytes(buffer, toRead);
      
      if(readCount == 0) {
          break;
      }
      
      LCDspi.transferBytes(buffer, NULL, readCount);
      remaining -= readCount;
  }
